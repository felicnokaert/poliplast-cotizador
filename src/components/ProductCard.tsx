...
function CatalogRow({ product, variant, onAdd }: { product: ProductWithVariants; variant: VariantWithPricing; onAdd?: (variant: VariantWithPricing, product: ProductWithVariants) => void }) {
  ...
  return (
    <div className="catalog-row">
      <div className="catalog-product" title={product.name}><i aria-hidden="true" /><span><strong>{product.name}</strong><small>{product.brand} · {product.family}{product.subfamily ? ` › ${product.subfamily}` : ''}</small>{variant.internal_note && <small className="product-note">ⓘ {variant.internal_note}</small>}{variant.hasTechnicalDoc && <button className="technical-link" onClick={openDocument}>Ficha técnica verificada</button>}</span></div>
