...
export function AdminPanel({ userEmail = '' }: { userEmail?: string }) {
  ...
  return <main className="admin-page">
    <div className="page-intro"><span className="eyebrow">Solo administradores</span><h1>Administración</h1><p className="muted">Productos, precios y reglas comerciales organizados en un solo lugar.</p></div>
    <nav className="admin-tabs" aria-label="Secciones de administración"><button className={section === 'catalogo' ? 'active' : ''} onClick={() => setSection('catalogo')}>Productos y precios</button><button className={section === 'politicas' ? 'active' : ''} onClick={() => setSection('politicas')}>Políticas y pagos</button><button className={section === 'masivo' ? 'active' : ''} onClick={() => setSection('masivo')}>Importar / exportar</button><button className={section === 'stock' ? 'active' : ''} onClick={() => setSection('stock')}>Stock</button></nav>
    ...
    {section === 'stock' && <><div className="admin-metrics"><article><span>SKU con stock aprobado</span><strong>{data.inventory.length}</strong><small>Con fecha de último conteo</small></article><article><span>Depósitos activos</span><strong>{data.locations.filter((location) => location.active).length}</strong><small>Separados por ubicación</small></article></div><section className="admin-section stack"><div><h2>Stock por depósito</h2><p className="muted">El stock se registra por depósito, se cuenta y luego se aprueba. Una cotización sólo consulta disponibilidad.</p></div><div className="location-grid">{data.locations.filter((location) => location.active).length === 0 ? <span className="pending-control">Todavía no hay depósitos activos configurados.</span> : data.locations.filter((location) => location.active).map((location) => { const balances = data.inventory.filter((item) => item.location_id === location.id); return <article key={location.id}><span>{location.code}</span><strong>{location.name}</strong><small>{balances.length} SKU con saldo aprobado</small></article> })}</div><p className="muted">Próximo flujo: exportar plantilla por depósito → cargar conteo → revisar diferencias → aprobar saldo.</p></section></>}
    {section === 'masivo' && <><div className="admin-metrics"><article><span>Revisiones de costo</span><strong>{data.costs.length}</strong><small>{currentCosts.length} vigentes</small></article><article><span>Últimos lotes</span><strong>{data.imports.length}</strong><small>Trazabilidad</small></article></div><section className="admin-section"><div><h2>Exportar</h2><p className="muted">Descargá una matriz editable por SKU.</p></div><div className="admin-actions"><button disabled={!data.catalog.length} onClick={() => downloadCsv(`catalogo-admin-poliplast-${new Date().toISOString().slice(0, 10)}.csv`, rowsToCsv(data.catalog))}>Matriz completa</button><button className="secondary" disabled={!data.costs.length} onClick={() => downloadCsv(`costos-poliplast-${new Date().toISOString().slice(0, 10)}.csv`, rowsToCsv(data.costs))}>Sólo costos</button></div></section><section className="admin-section stack"><div><h2>Importar cambios</h2><p className="muted">Primero se valida; costos y precios se aplican por separado.</p></div><label className="csv-drop">Seleccionar CSV<input type="file" accept=".csv,text/csv" onChange={(event) => inspectFile(event.target.files?.[0])} /></label>{preview.length > 0 && <div className="import-preview"><div className="import-summary"><strong>{previewFile}</strong><span className="change">{previewSummary.changes} cambios</span><span className="error">{previewSummary.errors} errores</span><span>{previewSummary.unchanged} sin cambios</span></div>{previewSummary.errors > 0 && <p className="quote-warning">Corregí todos los errores antes de aplicar.</p>}<div className="preview-rows">{preview.filter((row) => row.status !== 'sin_cambios').slice(0, 50).map((row) => <div key={`${row.row}-${row.sku}`} className={`preview-row ${row.status}`}><strong>Fila {row.row} · {row.sku || 'sin SKU'}</strong><span>{[...row.errors, ...row.changes].join(' · ')}</span></div>)}</div><div className="price-import-settings"><label>Lista<select value={priceKind} onChange={(event) => setPriceKind(event.target.value as PriceImportKind)}><option value="consumidor_final">Consumidor final</option><option value="mayorista">Mayorista</option></select></label><label>Versión<input value={priceListName} onChange={(event) => setPriceListName(event.target.value)} /></label><label>Moneda<select value={priceCurrency} onChange={(event) => setPriceCurrency(event.target.value as 'ARS' | 'USD')}><option>USD</option><option>ARS</option></select></label><label>Desde<input type="date" value={priceValidFrom} onChange={(event) => setPriceValidFrom(event.target.value)} /></label></div><div className="admin-actions"><button disabled={previewSummary.errors > 0 || !costRows.length || applying} onClick={() => setConfirming('costos')}>Revisar {costRows.length} costos</button><button disabled={previewSummary.errors > 0 || !priceRows.length || !priceListName.trim() || applying} onClick={() => setConfirming('precios')}>Revisar {priceRows.length} precios</button></div></div>}</section>{result && <section className="admin-section"><div><h2>Lote aplicado</h2><p className="muted">{result.applied} revisiones · {result.jobId}</p></div><button className="secondary" disabled={applying} onClick={revertLast}>Revertir lote</button></section>}</>}
